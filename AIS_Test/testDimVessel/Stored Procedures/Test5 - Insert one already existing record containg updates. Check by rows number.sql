...
-- Test5 - Insert one already existing record containg updates.
...
CREATE PROCEDURE [testDimVessel].[Test5 - Insert one already existing record containg updates. Check by rows number]
AS
BEGIN
 DECLARE @rowCount_Dim_Vessel int
  ...
  --Prepare Dim_Vessel to be pre-populated with the record
  ...
  /*ACT*/
	-- ETL for Dim_Vessel to insert the following CSV. file record, consisting of an already existing record in Dim_Vessel (where MMSI = 219013485) with updates
 ...
 -- exec tsqlt.Run @TestName = '[testDimVessel].[Test5 - Insert one already existing record containg updates. Check by rows number]'
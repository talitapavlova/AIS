...
/******
	...
	Test03 - Insert records containing both Vessel and Voyage Info(message type 5, 24 or 19), followed by route updates(message type 1, 3, 18).
		     The test performs the ETL targeting Fact_route, as well as other dependent stored procedures. 
			 The source file contains a message of type5 providing both Vessel and Voyage information. 
			 The other messages are of differnt types(3,1,18) holding dynamic information about the route of the vessel
			 The test verifies if Fact_Route cross-references the relevant vessel and voyage records and moreover checks if the correct updates are inserted, 
			 by assessing if the expected values match the actual Fact_Route
			 
*/

CREATE   PROCEDURE [testFactRoute].[Test03 - Insert records containing both Vessel and Voyage Info, followed by updates for the same vessel.]
AS
BEGIN
/*ARRANGE*/ 
...
/*ACT*/
	...
	execute [etl].Run_ETL 1, 'C:\AIS\Tests\test_FactRoute\Test3.csv'
...
/*ASSERT*/ 
  ...
  -- evaluate if the expected table matches the actual table
   ...
   --exec tsqlt.Run @TestName = '[testFactRoute].[Test03 - Insert records containing both Vessel and Voyage Info, followed by updates for the same vessel.]'